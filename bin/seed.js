...
mongoose   // make sure that you connect to the same DB as in "app.js"
  .connect('mongodb://localhost/express-library-project', {useMongoClient: true})
  .then(() => {
...
bookData.forEach((oneBook) => {
  Author.create(oneBook.author)
    .then((authorDoc) => {
      console.log(`Created AUTHOR ${authorDoc.firstName} ${authorDoc.lastName}`);

      // replace the author object with the new ObjectId
